...
class ListFilesTool:
    ...
    def run(self, arguments: dict[str, Any], context: RuntimeContext) -> Observation:
        ...
        for extra in arguments.get("ignore_dirs", []) or []:
        ...
        while queue and len(files) < limit:
            ...
            for entry in entries:
                name = entry.name
                if entry.is_dir():
                    if not include_hidden and (name.startswith(".") or name in ignore_dirs):
                        continue
                    queue.append(entry)
                    continue
                if not include_hidden and name.startswith("."):
                    continue
                try:
                    relative_path = entry.relative_to(context.working_dir)
                except ValueError:
                    relative_path = entry
